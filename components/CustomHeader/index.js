  ...
  return (
    <Header className="relative bg-blue-600">
      <Row className="flex flex-1 relative">
        <Link href="/">
          <p className="text-gray font-bold text-gray-100 font-sans text-2xl text-center flex justify-center items-center cursor-pointer mt-4">
            BilsemAI
          </p>
        </Link>
        {loggedIn && (
          <Dropdown overlay={menu} trigger={["click"]}>
            <div className=" absolute right-10 top-2 pl-4 pr-4 h-12 flex items-center cursor-pointer">
              <UserOutlined className="text-lg w-10 h-10 flex justify-center items-center rounded-full bg-transparentWhite" />
              <span className="text-gray font-bold ml-2 mr-2 text-gray-100 select-none">
                {staff?.nickname}
              </span>
              <FontAwesomeIcon
                className="text-gray"
